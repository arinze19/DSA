import { twoNumberSum, twoNumberSum2 } from './twoNumberSum';
...
test('Determine If array contains elements that add up to target sum', () => {
    ...
    expect(twoNumberSum2([3, 5, -4, 8, 11, 1, -1, 6], 10)).toEqual([11, -1])
    expect(twoNumberSum2([4, 6], 10)).toEqual([4, 6])
    expect(twoNumberSum2([3, 5, -4, 8, 11, 1, -1, 6], 15)).toEqual([])
    expect(twoNumberSum2([14], 10)).toEqual([])
    expect(twoNumberSum2([15], 15)).toEqual([])
    expect(twoNumberSum2([4, 6, 1, -3], 3)).toEqual([6, -3])
})